...
export const getWorkspaceByJoinCodeService = async (joinCode, userId) => {
  try {
    ...
    console.log('Get workspace service join code error', error);
    throw error;
  }
}
...
export const resetWorkspaceJoinCodeService = async (workspaceId, userId) => {
  try {
    ...
    console.log('Reset workspace join code error',error);
    throw error;
  }
}
...
export const addChannelToWorkspaceService = async (workspaceId, channelName, userId) => {
  try {
    ...
    console.log('add channel to workspace service', workspace, userId);    
    const isAdmin = isUserAdminOfWorkspace(workspace, userId);
    if(!isAdmin) {
      throw new ClientError({
        explanation: 'User is not an admin of the workspace',
    ...
    console.log('add channel to workspace service', workspaceId, channelName);

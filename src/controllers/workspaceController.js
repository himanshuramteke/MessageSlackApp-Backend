...
export const getWorkspaceByJoinCodeController = async (req, res) => {
    try {
        ...
        console.log('Get workspace by join code controller',error);
        if(error.statusCode) {
            return res.status(error.statusCode).json(customErrorResponse(error));
        }
...
export const addMemberToWorkspaceController = async (req, res) => {
    try {
       const response = await addMemberToWorkspaceService(
        req.params.workspaceId, req.body.memberId, req.body.role || 'member', req.user
       ); 
       return res
          .status(StatusCodes.OK)
          .json(
            successResponse(response, 'Member addded to workspace successfully')
        );
    } catch (error) {
...
export const verifyEmailController = async (req, res) => {
    try {
        ...
        return res
          .status(StatusCodes.OK)
          .json(successResponse(response, 'Email verified succeessfully'));
    } catch (error) {
